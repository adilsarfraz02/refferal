...
export default function Navbar() {
  ...
  return (
    <motion.header
      ...
      animate={{ y: 0 }}
      id='homeHeader'
      className='fixed top-0 w-full text-primary-foreground py-4 px-6 flex items-center justify-between'>
      <Link
      ...
      {status === 'loading' && (
        <button
          onClick={() => onOpenChange(true)}
          className='inline-flex animate-pulse hover:shadow-2xl h-10 w-20 hover:shadow-zinc-700/20 bg-zinc-500/10 relative items-center justify-center whitespace-nowrap text-base font-sm transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-zinc-500/20 rounded-3xl px-4 py-2'></button>
      )}
